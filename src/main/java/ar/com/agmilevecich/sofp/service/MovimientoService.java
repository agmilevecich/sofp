...
public class MovimientoService {
    ...
    public Movimiento registrar(Cuenta cuenta, Categoria categoria, Moneda moneda,
        ...
        validarSaldoDisponible(cuenta, moneda, tipoMovimiento, importe, formaPago, null, fechaHora);
    ...
    public Movimiento modificarTipoMovimiento(Long movimientoId, Long usuarioId, TipoMovimiento tipoMovimiento) {
        ...
        validarSaldoDisponible(movimiento.getCuenta(), movimiento.getMoneda(), tipoMovimiento, movimiento.getImporte(), movimiento.getFormaPago(), movimiento, movimiento.getFechaHora());
    ...
    public Movimiento modificarImporte(Long movimientoId, Long usuarioId, BigDecimal importe) {
        ...
        validarMovimientoIndependiente(movimiento);
        validarMovimientoSinObligacion(movimiento);
        validarSaldoDisponible(movimiento.getCuenta(), movimiento.getMoneda(), movimiento.getTipoMovimiento(), importe, movimiento.getFormaPago(), movimiento, movimiento.getFechaHora());
    ...
    private void validarSaldoDisponible(Cuenta cuenta, Moneda moneda, TipoMovimiento tipoMovimiento,
                                        ...
                                        Movimiento movimientoActual, LocalDateTime fechaHora) {
        if (tipoMovimiento != TipoMovimiento.EGRESO || formaPago == FormaPago.TARJETA_CREDITO) return;

        BigDecimal saldoDisponible = calcularSaldo(cuenta.getId(), moneda, fechaHora);
    ...
    private BigDecimal calcularSaldo(Long cuentaId, Moneda moneda, LocalDateTime fechaHora) {
        BigDecimal saldo = BigDecimal.ZERO;
        for (Movimiento movimiento : movimientoRepository.listarPorCuentaHastaFecha(cuentaId, fechaHora)) {
