...
class CicloFacturacionTest {
    ...
    void deberiaAsignarConsumoAnteriorAlCierreAlCicloActual() {
        ...
        CicloFacturacion ciclo = tarjeta.calcularCicloFacturacion(LocalDate.of(2026, 9, 5));
    ...
    void deberiaAsignarConsumoDelDiaDeCierreAlCicloQueCierraEseDia() {
        Cuenta tarjeta = crearTarjeta(10, 25);
        CicloFacturacion ciclo = tarjeta.calcularCicloFacturacion(LocalDate.of(2026, 9, 10));
        assertEquals(LocalDate.of(2026, 8, 11), ciclo.getFechaInicio());
        assertEquals(LocalDate.of(2026, 9, 10), ciclo.getFechaCierre());
        assertEquals(LocalDate.of(2026, 9, 25), ciclo.getFechaVencimiento());
    }
    ...
    void deberiaAsignarConsumoPosteriorAlCierreAlCicloSiguiente() {
        Cuenta tarjeta = crearTarjeta(10, 25);
        CicloFacturacion ciclo = tarjeta.calcularCicloFacturacion(LocalDate.of(2026, 9, 11));
        assertEquals(LocalDate.of(2026, 9, 11), ciclo.getFechaInicio());
        assertEquals(LocalDate.of(2026, 10, 10), ciclo.getFechaCierre());
        assertEquals(LocalDate.of(2026, 10, 26), ciclo.getFechaVencimiento());
    }
    ...
    void deberiaLlevarElVencimientoAlMesSiguienteCuandoCoincideConElCierre() {
        ...
        CicloFacturacion ciclo = tarjeta.calcularCicloFacturacion(LocalDate.of(2026, 9, 5));
        assertEquals(LocalDate.of(2026, 9, 10), ciclo.getFechaCierre());
        assertEquals(LocalDate.of(2026, 10, 12), ciclo.getFechaVencimiento());
    }

    @Test
    void deberiaAjustarCierreAlUltimoDiaRealDeFebrero() {
        Cuenta tarjeta = crearTarjeta(31, 25);
        CicloFacturacion ciclo = tarjeta.calcularCicloFacturacion(LocalDate.of(2026, 2, 20));
    ...
    void deberiaAjustarVencimientoAlUltimoDiaRealDelMes() {
        ...
        CicloFacturacion ciclo = tarjeta.calcularCicloFacturacion(LocalDate.of(2026, 9, 5));
        assertEquals(LocalDate.of(2026, 9, 10), ciclo.getFechaCierre());
        assertEquals(LocalDate.of(2026, 11, 2), ciclo.getFechaVencimiento());
    }

    @Test
    void deberiaResolverCorrectamenteElCambioDeAnio() {
        Cuenta tarjeta = crearTarjeta(10, 25);
        CicloFacturacion ciclo = tarjeta.calcularCicloFacturacion(LocalDate.of(2026, 12, 11));
    ...
    void deberiaAsignarConsumoAlCicloActualCuandoOcurreExactamenteEnElDiaDeCierre() {
        ...
        CicloFacturacion ciclo = tarjeta.calcularCicloFacturacion(LocalDate.of(2026, 9, 15));
    ...
    void deberiaAsignarConsumoDelDiaPosteriorAlCierreAlCicloSiguiente() {
        Cuenta tarjeta = crearTarjeta(15, 5);
        CicloFacturacion ciclo = tarjeta.calcularCicloFacturacion(LocalDate.of(2026, 9, 16));
    ...
    void noDeberiaPermitirFechaDeConsumoNula() {
        Cuenta tarjeta = crearTarjeta(10, 25);
        assertThrows(NullPointerException.class, () -> tarjeta.calcularCicloFacturacion(null));
    }

    private Cuenta crearTarjeta(int diaCierre, int diaVencimiento) {
        Usuario usuario = new Usuario("Ariel", "Usuario", "ariel@test.com", "hash");
        PerfilFinanciero perfil = new PerfilFinanciero("Personal", usuario);
        InstitucionFinanciera institucion = new InstitucionFinanciera("Banco Santander", TipoInstitucionFinanciera.BANCO);
        Moneda moneda = new Moneda("ARS", "Peso Argentino", 2, TipoMoneda.FIAT);
        return new Cuenta("Visa Santander", perfil, institucion, moneda, new BigDecimal("500000.00"), diaCierre, diaVencimiento);
    }
}
